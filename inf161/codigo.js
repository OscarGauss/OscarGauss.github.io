...
//161
function crear_tabla_cerr(nom){
	var n = document.getElementById(nom+"ndf").value;
    for(var i=1; i<=n; i++){
		var pp= document.createElement("p");
		var ip1=document.createElement("input");
		var at11 = document.createAttribute("type"); at11.value="text"; ip1.setAttributeNode(at11);
		var at12 = document.createAttribute("name"); at12.value=nom+"I"; ip1.setAttributeNode(at12);
		var at13 = document.createAttribute("maxlength"); at13.value="15"; ip1.setAttributeNode(at13);
		var at14 = document.createAttribute("size"); at14.value="15"; ip1.setAttributeNode(at14);
		var at15 = document.createAttribute("id"); at15.value=nom+"I"+i; ip1.setAttributeNode(at15);
		var ip2=document.createElement("input");
		var at21 = document.createAttribute("type"); at21.value="text"; ip2.setAttributeNode(at21);
		var at22 = document.createAttribute("name"); at22.value=nom+"D"; ip2.setAttributeNode(at22);
		var at23 = document.createAttribute("maxlength"); at23.value="15"; ip2.setAttributeNode(at23);
		var at24 = document.createAttribute("size"); at24.value="15"; ip2.setAttributeNode(at24);
        var at25 = document.createAttribute("id"); at25.value=nom+"D"+i; ip2.setAttributeNode(at25);
		var t = document.createTextNode("→");       // Create a text node
		pp.appendChild(ip1); pp.appendChild(t); pp.appendChild(ip2);
		document.getElementById(nom).appendChild(pp);
...
//161
function limpiar_cerr(nom){
	document.getElementById(nom+"atrib").value="";
	document.getElementById(nom+"cerr").value="";
    var n=document.getElementById(nom+"ndf").value;
    document.getElementById(nom+"ndf").value="";
    for(var i=0; i<n; i++)
		document.getElementById(nom).removeChild(document.getElementById(nom).getElementsByTagName('p')[3]);
}
function guardar_relacion(form, car){
	var A = document.getElementById(form+"atrib").value.split(car);
	for(var i=0; i<A.length; i++) A[i]=A[i].trim();
	var ndf = document.getElementById(form+"ndf").value;
	var aux=[];
	for(var i=0; i<ndf; i++){
		aux.push({x:document.getElementById(form+"I"+(i+1)).value.trim(),
				  y:document.getElementById(form+"D"+(i+1)).value.trim()});
	}	
	return {A:A, DF:aux};
}
function procesar_cerradura(nom1, nom2){
	...
	var sw = true;
	var LOL=0;
	while(sw){ LOL++; if(LOL>50) break;
		sw = false;
		for(var i=0; i<R.DF.length; i++){ //alert("RDi: "+R.DF[i].x);
			var cnt=0;
			for(var j=0; j<R.DF[i].x.length; j++){
				//alert(R.DF[i].x.charAt(j));
				if( is_in(X,R.DF[i].x.charAt(j))  ) cnt++;
			}			
			if(R.DF[i].x.length==cnt){
				for(var j=0; j<R.DF[i].y.length; j++){
					if( !is_in(X,R.DF[i].y.charAt(j))  ){
						//alert("insertar: "+R.DF[i].y.charAt(j));
						sw = true;
						X.push(R.DF[i].y.charAt(j));
					}
				}
			}
		}
	...
	document.getElementById(nom2+"cerr").value=X.join(",");
